...
export default function MerchantError({ error, reset }) {
  useEffect(() => { console.error("[merchant]", error); }, [error]);
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#F7F7FB] p-4">
      <div className="mx-auto flex max-w-md flex-col items-center gap-4 rounded-2xl border border-[#E5E7EB] bg-white p-8 text-center shadow-sm">
        ...
        <p className="text-sm text-gray-500">Mağaza panelinde beklenmeyen bir hata oluştu.</p>
        <div className="flex gap-3">
          <button
            onClick={() => reset()}
            className="rounded-full bg-[#6C3BFF] px-5 py-2.5 text-sm font-bold text-white hover:bg-[#582CD6]"
          >
            Tekrar dene
          </button>
          <a
